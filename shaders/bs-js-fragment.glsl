...
// Base the color on the number of iterations
     ...
     else
     {
          ...
          pi = 3.141592654;
           ...
           // fi = sin(0.25*(iter/40.0));
           // red = clamp(zx * fi * zy, 0.0, 1.0);
           // green = clamp(abs(sin(zy * zy * red)), 0.0, 1.0) ;
           // blue = clamp(abs(sin(zx * fi)), 0.0, 1.0);

           // red =   clamp(fi * fi * abs(sin(pow(fi, pos.x))), 0.0, 1.0);
           // green = clamp(abs(sin(pow(fi, zy - zx))), 0.0, 1.0);
           // blue =  clamp(fi * abs(cos(pow(fi, fi)) * sin(pow(zx, fi))), 0.0, 1.0);
